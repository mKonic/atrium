...
// Where a window in a secret space (or a tiled one) goes: the screen less a
// margin of `percent` of its shorter side, the same on every side, so the
// blurred desktop shows around the window.
